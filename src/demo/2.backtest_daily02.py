# 回测演示02：策略是5个交易日平均成交量最大的股票
# 生成 账户净值 (Equity) 和 实际资金 (Value)资产走势图
...
if __name__ == '__main__':
	...
	# 6. 可视化分析
	# 计算账户净值 (Equity) 和 实际资金 (Value)
	report.index = pd.to_datetime(report.index)
	report['cum_ret'] = (1 + report['return']).cumprod()  # report['return'] 是每日收益率
	report['account_value'] = start_cash * report['cum_ret']
	# 绘图
	plt.figure(figsize=(12, 6))
	report['account_value'].plot(color='blue', linewidth=2)
	plt.title('Account Value Trend - 2025 (Based on Volume Strategy)', fontsize=14)
	plt.xlabel('Date', fontsize=12)
	plt.ylabel('Total Asset (CNY)', fontsize=12)
	plt.grid(True, linestyle='--', alpha=0.7)
	plt.axhline(y=start_cash, color='red', linestyle='-', label='Initial Capital')  # 红线代表初始本金
	plt.legend()
	plt.tight_layout()
	plt.show()
